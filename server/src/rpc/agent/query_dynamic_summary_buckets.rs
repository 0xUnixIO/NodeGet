...
use nodeget_lib::monitoring::query::{DynamicSummaryBucketsQuery, DynamicSummaryQueryField};
...
pub async fn query_dynamic_summary_buckets(
    token: String,
    ...
    let process_logic = async {
        debug!(
            ...
            "Dynamic summary buckets query requested"
        );
        ...
        let uuid_id = uuid_cache.get_id(&query.uuid).await.ok_or_else(|| {
            NodegetError::NotFound(format!(
                "Agent UUID {} not found in monitoring registry",
                query.uuid
            ))
        })?;
        ...
        let sql = build_sql(&query.fields);
        ...
        let row = JsonAggRow::find_by_statement(statement)
            ...
            .await
            .map_err(|e| {
                error!(target: "monitoring", error = %e, "Dynamic summary buckets DB error");
...
fn ensure_postgres(db: &DatabaseConnection) -> anyhow::Result<()> {
    ...
    Err(NodegetError::InvalidInput(
        "agent_query_dynamic_summary_buckets only supports PostgreSQL".to_owned(),
    )
    .into())
}

fn build_sql(fields: &[DynamicSummaryQueryField]) -> String {
    // raw CTE：每桶的 COUNT + 各字段 AVG（scaled 字段除以 10 还原）
    let field_aggregates = fields.iter().fold(String::new(), |mut s, f| {
        if f.is_scaled() {
            write!(
                s,
                ",\n        AVG({col})::double precision / 10.0 AS {col}",
                col = f.column_name()
            )
            .unwrap();
        } else {
            write!(
                s,
                ",\n        AVG({col})::double precision AS {col}",
                col = f.column_name()
            )
            .unwrap();
        }
